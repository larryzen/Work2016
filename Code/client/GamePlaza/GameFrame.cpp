	...
	case ItemGenre_Inside:
		{
			...
			WebBrowse(TEXT("http://127.0.0.1"),false);
	...
	case ItemGenre_Kind:		//游戏类型
		{
			...
			_snprintf(szRuleUrl,sizeof(szRuleUrl),TEXT("http://127.0.0.1/GameRule.asp?KindID=%ld"),pGameKind->wKindID);
...
BOOL CGameFrame::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_BT_BUTTON_5:				//功能按钮
		{
			if(IDC_BT_BUTTON_1==nCommandID)WebBrowse(TEXT("http://127.0.0.1/index.asp"),true);
			if(IDC_BT_BUTTON_2==nCommandID)WebBrowse(TEXT("http://127.0.0.1/PayBuy.asp"),true);
			if(IDC_BT_BUTTON_4==nCommandID)WebBrowse(TEXT("http://127.0.0.1/user/ExChange.asp"),true);
			if(IDC_BT_BUTTON_5==nCommandID)WebBrowse(TEXT("http://127.0.0.1/bbs/index.asp?action=frameon"),true);
...
//按键测试
HRESULT CGameFrame::OnNcHitTest(CPoint Point)
{
	//按钮测试
	if (m_bMaxShow==false)
	{
